...
const targetOrigin=targetFrame.p.clone().addScaledVector(targetT,52); targetOrigin.y-=14;
...
function reset(){s=TEST_STAGE2?total-85:checkpoint;v=TEST_STAGE2?18:26;theta=omega=input=holdTime=0;dead=false;stage=1;score=0;impactDone=false;fail.style.display='none';for(const b of blocks){b.active=false;b.hit=false;b.vel.set(0,0,0);b.spin.set(0,0,0)}}
function launchStage2(p,t,rr){stage=2;dead=false;impactDone=false;launchVel.copy(t).multiplyScalar(Math.min(v,22)).addScaledVector(rr,omega*R*1.2);launchVel.y=24;rider.position.copy(p).addScaledVector(t,4.5).add(new THREE.Vector3(0,1.2,0));prog.textContent='DESTROY · 0';}
